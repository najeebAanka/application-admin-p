        ...
        <div class="row pt-4" style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};">
            <div class="col-md-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="{{route('admin.banner.update',[$kitchen['id']])}}" method="post"
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select style="width: 100%"
                                                ...
                                                <option value="Main Banner" {{$kitchen['banner_type']=='Main Banner'?'selected':''}}>
                                                    Main Banner
                                                </option>
                                                <option value="Footer Banner" {{$kitchen['banner_type']=='Footer Banner'?'selected':''}}>
                                                    Footer Banner
                                                </option>
                                                <option value="Offer Banner" {{$kitchen['banner_type']=='Offer Banner'?'selected':''}}>
                                                    Offer Banner
                                                </option>
                                                <option value="Home Section" {{$kitchen['banner_type']=='Home Section'?'selected':''}}>
                                                    Home Section
                                                </option>
                                                <option value="Account Banner" {{$kitchen['banner_type']=='Account Banner'?'selected':''}}>
                                                    Account Banner
                                                </option>
                                                <option value="Cart Banner" {{$kitchen['banner_type']=='Cart Banner'?'selected':''}}>
                                                    Cart Banner
                                                </option>
                                            </select>
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select style="width: 100%" onchange="display_data(this.value)"
                                                ...
                                                <option value="product" {{$kitchen['resource_type']=='product'?'selected':''}}>
                                                    Product
                                                </option>
                                                <option value="category" {{$kitchen['resource_type']=='category'?'selected':''}}>
                                                    Category
                                                </option>
                                                {{--                                                <option value="shop" {{$kitchen['resource_type']=='shop'?'selected':''}}>Shop</option>--}}
                                                <option value="brand" {{$kitchen['resource_type']=='brand'?'selected':''}}>
                                                    Brand
                                                </option>
                                            </select>
                                        </div>
                                        ...
                                        <div class="form-group" id="resource-product"
                                             style="display: {{$kitchen['resource_type']=='product'?'block':'none'}}">
                                            <label for="product_id">{{\App\CPU\translate('product')}}</label>
                                            <select style="width: 100%"
                                                    class="js-example-responsive form-control"
                                                    name="product_id">
                                                @foreach(\App\Model\Product::active()->get() as $product)
                                                    <option value="{{$product['id']}}" {{$kitchen['resource_id']==$product['id']?'selected':''}}>{{$product['name']}}</option>
                                                @endforeach
                                            </select>
                                        </div>

                                        <div class="form-group" id="resource-category"
                                             style="display: {{$kitchen['resource_type']=='category'?'block':'none'}}">
                                            <label for="name">{{\App\CPU\translate('category')}}</label>
                                            <select style="width: 100%"
                                                    class="js-example-responsive form-control"
                                                    name="category_id">
                                                @foreach(\App\CPU\CategoryManager::parents() as $category)
                                                    <option value="{{$category['id']}}" {{$kitchen['resource_id']==$category['id']?'selected':''}}>{{$category['name']}}</option>
                                                @endforeach
                                            </select>
                                        </div>

                                        <div class="form-group" id="resource-shop"
                                             style="display: {{$kitchen['resource_type']=='shop'?'block':'none'}}">
                                            <label for="shop_id">{{\App\CPU\translate('shop')}}</label>
                                            <select style="width: 100%"
                                                    class="js-example-responsive form-control"
                                                    name="shop_id">
                                                @foreach(\App\Model\Shop::active()->get() as $shop)
                                                    <option value="{{$shop['id']}}" {{$kitchen['resource_id']==$shop['id']?'selected':''}}>{{$shop['name']}}</option>
                                                @endforeach
                                            </select>
                                        </div>

                                        <div class="form-group" id="resource-brand"
                                             style="display: {{$kitchen['resource_type']=='brand'?'block':'none'}}">
                                            <label for="brand_id">{{\App\CPU\translate('brand')}}</label>
                                            <select style="width: 100%"
                                                    class="js-example-responsive form-control"
                                                    name="brand_id">
                                                @foreach(\App\Model\Brand::all() as $brand)
                                                    <option value="{{$brand['id']}}" {{$kitchen['resource_id']==$brand['id']?'selected':''}}>{{$brand['name']}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    ...
                                    <div class="col-md-6">
                                        <center>
                                            <img
                                                    ...
                                                    id="mbImageviewer"
                                                    src="{{asset('storage/banner')}}/{{$kitchen['photo']}}"
                                                    alt="banner image"/>
                                        </center>
                                    </div>
