...
class SocialAuthController extends Controller
{
    public function social_login(Request $request)
    {
        ...
        try {
            ...
            return Helpers::sendError([['message' => translate('wrong credential.')]], $exception->getCode());
        }
        ...
        if (strcmp($email, $data['email']) === 0) {
            ...
            if (!isset($user->phone)) {
                return Helpers::sendSuccess(translate('update phone number'), [
            ...
            if ($token != null) {
                return Helpers::sendSuccess(translate('Data Got!'), ['token' => $token]);
            }
            return Helpers::sendSuccess(translate('Customer_not_found_or_Account_has_been_suspended'), '');
        }
        return Helpers::sendSuccess(translate('email_does_not_match'), '');
    }
    ...
    public function update_phone(Request $request)
    {
        ...
        if ($phone_verification->value == 1) {
            return Helpers::sendSuccess(translate('phone verification on'), [
                'token_type' => 'phone verification on',
                'temporary_token' => $request->temporary_token
            ]);
        } else {
            return Helpers::sendSuccess(translate('Phone number updated successfully'), '');
        }
    }
