...
//        Log::info($request->all());
        ...
        if ($request->user_type == 1) {
            ...
            if ($phone_verification && !$user->is_phone_verified) {
                return Helpers::sendSuccess(translate('Data Got!'), ['temporary_token' => $temporary_token]);
            }
            ...
            try {
                $w = new CustomerWallet();
                $w->customer_id = $user->id;
                $w->balance = 0;
                $w->royality_points = 500;
                $w->save();
            } catch (\Exception $ex) {
    ...
    public function login(Request $request)
    {
        ...
        if (filter_var($user_id, FILTER_VALIDATE_EMAIL)) {
            ...
            if ($count >= 9 && $count <= 15) {
                ...
                return Helpers::sendError([['code' => 'email', 'message' => translate('Invalid email address or phone number')]], 501);
            }
        }
        ...
        if (isset($user) && $user->is_active && auth()->attempt($data)) {
            ...
            $email_verification = Helpers::get_business_settings('email_verification');
            if ($phone_verification && !$user->is_phone_verified) {
                return Helpers::sendSuccess(translate('Data Got!'), ['temporary_token' => $user->temporary_token]);
            }
            if ($email_verification && !$user->is_email_verified) {
                return Helpers::sendSuccess(translate('Data Got!'), ['temporary_token' => $user->temporary_token]);
            }
            ...
            if (auth('seller')->
            attempt(['email' => $request->email, 'password' => $request->password, 'status' => 'approved'], true)) {
                $seller = Seller::where('email', $request->email)->first();
                $seller->auth_token = md5(time());
                $seller->update();
                return Helpers::sendSuccess(translate('Data Got!'), [
                    'user_type' => 2,
                    'token' => $seller->auth_token,
                    'user' => \App\Resources\User::make($seller)
                ]);
            }
    ...
    public function logout(Request $request)
    {
        ...
        return Helpers::sendSuccess(translate('Logged out!'), '');
    }
}
