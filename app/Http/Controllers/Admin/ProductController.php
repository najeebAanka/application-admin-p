...
class ProductController extends BaseController
{
    ...
    public function store(Request $request)
    {
        ...
        $p->features = $request->features[array_search('en', $request->lang)];
        ...
        if (count($combinations[0]) > 0) {
            foreach ($combinations as $key => $combination) {
                ...
                $item = [];
                $item['type'] = $str;
                $item['price'] = BackEndHelper::currency_to_aed(abs($request['price_' . str_replace('.', '_', $str)]));
        ...
        $p->unit_price = BackEndHelper::currency_to_aed($request->unit_price);
        $p->purchase_price = BackEndHelper::currency_to_aed($request->purchase_price);
        $p->tax = $request->tax_type == 'flat' ? BackEndHelper::currency_to_aed($request->tax) : $request->tax;
        $p->tax_type = $request->tax_type;
        $p->discount = $request->discount_type == 'flat' ? BackEndHelper::currency_to_aed($request->discount) : $request->discount;
        ...
        if ($request->ajax()) {
            ...
            foreach ($request->lang as $index => $key) {
                ...
                if ($request->features[$index] && $key != 'en') {
                    array_push($data, array(
                        'translationable_type' => 'App\Model\Product',
                        'translationable_id' => $p->id,
                        'locale' => $key,
                        'key' => 'features',
                        'value' => $request->features[$index],
                    ));
                }
            }
    ...
    public function update(Request $request, $id)
    {
        ...
        $product->features = $request->features[array_search('en', $request->lang)];
        ...
        //combinations end
        $product->variation = json_encode($variations);
        $product->unit_price = BackEndHelper::currency_to_aed($request->unit_price);
        $product->purchase_price = BackEndHelper::currency_to_aed($request->purchase_price);
        $product->tax = $request->tax == 'flat' ? BackEndHelper::currency_to_aed($request->tax) : $request->tax;
        $product->tax_type = $request->tax_type;
        $product->discount = $request->discount_type == 'flat' ? BackEndHelper::currency_to_aed($request->discount) : $request->discount;
