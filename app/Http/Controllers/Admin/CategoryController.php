...
class CategoryController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        if ($request->banner) {
            $category->banner = ImageManager::update('category/', $category->banner, 'png', $request->file('banner'));
        }
        $category->products_style = $request->products_style;
        $category->is_gift = $request->is_gift;
        $category->save();
    ...
    public function statusTop(Request $request)
    {
        $category = Category::find($request->id);
        $category->home_top = $request->home_top;
        $category->save();
        Toastr::success('Category status updated!');
        return back();
    }
    ...
    {
        $category = Category::find($request->id);
        $category->home_section = $request->home_section;
        $category->save();
        Toastr::success('Category status updated!');
        return back();
    }
    public function is_mad_test(Request $request)
    {
        $category = Category::find($request->id);
        $category->is_mad_test = $request->is_mad_test;
        $category->save();
        Toastr::success('Category is_mad_test now!');
        return back();
    }
}
