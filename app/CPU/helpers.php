...
class Helpers
{
    ...
    public static function currency_converter($amount, $symp = true)
    {
    ...
    public static function convert_currency_to_aed($price)
    {
        $currency_model = Helpers::get_business_settings('currency_model');
        if ($currency_model == 'multi_currency') {
            Helpers::currency_load();
            $code = session('currency_code') == null ? 'AED' : session('currency_code');
    ...
    public static function set_symbol($amount)
    {
        ...
        if (!is_null($position) && $position == 'left') {
            $string = translate(currency_symbol()) . ' ' . number_format($amount, 2);
        } else {
            $string = number_format($amount, 2) . ' ' . translate(currency_symbol());
        }
        return $string;
    }
    ...
    public static function sendError($errors, $code)
    {
