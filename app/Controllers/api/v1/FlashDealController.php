...
class FlashDealController extends Controller
{
    public function get_flash_deal()
    {
        try {
            ...
            return Helpers::sendSuccess("Data Got!", $flash_deals);
        } catch (\Exception $e) {
    ...
    public function get_products($deal_id)
    {
        ...
        if (count($p_ids) > 0) {
            return Helpers::sendSuccess('Data Got!', Helpers::product_data_formatting(Product::with(['rating'])->whereIn('id', $p_ids)->get(), true), '');
        }
        return Helpers::sendSuccess('Dta not found!', '');
    }
}
